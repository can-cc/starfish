...
export let renderDir = (dirPath, theme, outputPath, cb, pickup, name = 'index') => {
    ...
    fs.readdir(dirPath, (err, paths) => {
        fs.mkdir(outputPath, () => {
            ...
            let mapdone = new MapDone(() => {
                pickup.renderIndex(outputPath, name);
                cb();
            });
            ...
            let fileNames = files.filter((file) => {
                ...
                renderFile(path.join(dirPath, file), theme, (data, article) => {
                    ...
                    fs.writeFile(outputFilePath, data, (err) => {
                        ...
                        pickup.addArticle(fileNames, article, name);
                        done();
                    });
                });
            ...
            subDirs.map((subDir) => {
                ...
                renderDir(path.join(dirPath, subDir), theme,
                          path.join(outputPath, subDir), done, pickup, subDir);
            });
