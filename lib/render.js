...
export class RenderController {
    ...
    loadTemplates() {
        let tempPath = path.join(this.themePath, 'templates');
        let self = this;
    ...
    renderCategoryIndex(name, outputPath, articles, cb) {
        ...
        R.splitEvery(this.options.BLOG.CATEGORY_ARTICLE_NUMBER, sorted).forEach((pArticles, i) => {
            var html = ejs.render(this.templates['category'], {
                title: name,
                articles: sorted,
                currentPageN: i,
                pageN: pageN,
    ...
    renderCategory(category, cb) {
        ...
        async.each(category.articles, (articleInfo, asyncCb) => {
            parseToData(path.join(inputPath, articleInfo.fileName), (err, articleDoc) => {
                ...
                var result = ejs.render(self.templates['article'], articleInfo);
    ...
    renderIndex(cb) {
        ...
        var html = ejs.render(self.templates['index'], {
