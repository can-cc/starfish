...
export class RenderController {
    ...
    loadLangData() {
        ...
        let langFiles = fs.readdirSync(path.join(this.inputPath, 'lang'))
            .filter(filterDotFiles)
            .filter(isYaml);

        if( langFiles.indexOf(lang + '.yaml') ){
            this.langData = loadConfig(path.join(this.inputPath, 'lang', lang + '.yaml'));
        } else {
            this.langData = loadConfig(path.join(this.inputPath, 'lang', 'default.yaml'));
        }
    }
    ...
    static fixArticleUrl(content, relativeOutputPath) {
    ...
    renderCategoryIndex(name, outputPath, articles, cb) {
        ...
        let pageN = Math.ceil(sorted.length / this.options.BLOG.INDEX_ARTICLE_NUMBER);

        let i = 0;
        async.each(R.splitEvery(this.options.BLOG.CATEGORY_ARTICLE_NUMBER, sorted), (articleChunk, asyncCb) => {
            let data = {
                title: self.categorys[name].aliasName || name,
                articles: articleChunk,
                currentPageN: i,
                pageN: pageN,
                offsetN: this.options.BLOG.PAGING_START_N || this.themeConfig.PAGING_START_N
            };
            
            var html = self.renderTemp('category', data);

            let outputDir = i === 0 ? outputPath : path.join(outputPath, i + data.offsetN);
    ...
    renderCategory(category, cb) {
        ...
        async.each(category.articles, (articleInfo, asyncCb) => {
            parseToData(path.join(inputPath, articleInfo.fileName), (err, articleDoc) => {
                ...
                let content = RenderController.fixArticleUrl(articleDoc.content, relativeOutputPath),
            ...
            let outputPath = category.outputPath;
            self.renderCategoryIndex(category.name, outputPath, category.articles, R.noop);
            cb();
        });        
    }
    ...
    renderAllArticles(cb) {
        if( this.templates['all'] ){
            ...
            var categorys = R.values(self.categorys).map((category) => {
                return {
                    name: category.name,
                    indexUrl: path.join('/', category.relativeOutputPath, 'allarticles.html'),
                    number: category.articles.length
                };
            }).filter((category) => {
                return category.number > 0;
            ...
            _.chunk(allarticles, this.options.BLOG.INDEX_ARTICLE_NUMBER).map((articles, i) => {
                ...
                var html = ejs.render(self.templates['allarticles'], {
                    title: self.options.BLOG.NAME,
                    articles: articles,
                    categorys: categorys,
                    currentPageN: i,
                ...
                let outputPath = this.outputRoot + '/page/' + i;
