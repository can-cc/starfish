...
import {syncMappingDirs, fixArticleUrlAndCut, generateParsers, parseDocument, getPlugin} from './render-util';
...
export class RenderController {
  constructor(inputPath, outputRoot, options) { // TODO merge inputPath, outputRoot
    ...
    this.parsers = generateParsers();
    this.documentParserFn = parseDocument(this.parsers);
  }
  ...
  generateArticleToRenderData(category, documentFile) {
  ...
  async renderCategory(category) {
    ...
    const xx = await Promise.all(category.articles.map(async (articleInfo) => {
    ...
    category.articles = await Promise.all(category.articles.map(this.generateArticleToRenderData));
