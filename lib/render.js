...
export class RenderController {
  ...
  async loadDir (dirPath, outputPath, category, cb) {
    const self = this;
    const paths = await pfs.readdirAsync(dirPath);
    ...
    //async.each()
    const fileNames = files.filter(self.filterIgnores.bind(self))
          .filter((file) => filterDotFiles(file) && R.values(parsers).some(parser => parser.check(file)));

    fileNames.map(async (fileName) => {
      await self.addArticle(fileName, category);
    });
