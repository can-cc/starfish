  ...
  fs.readdirSync(path.resolve(__dirname, '../node_modules'))
    .forEach((name) => {
      if( !self.plugins[name] ){
        let plugins = new (require(path.resolve(__dirname, '../command', name)).default)();
        if( plugins.type === type ){
          self.plugins[plugins.getName()] = plugins;
        }
      } else {
        throw new Error('duplicate plugins');
      }
    });
  return plugins;
};
...
export class RenderController {
  ...
  renderCategory(category, cb) {
    ...
    async.each(category.articles, (articleInfo, asyncCb) => {
      ...
      new Promise((resovle) => {
        fs.readFile(filePath, 'utf-8', resovle, error => asyncCb(error)); // 如果错误直接调用 async(error, null);
      }).then(articleRawData => {
