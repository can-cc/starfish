...
export class RenderController {
  ...
  runPluinEachArticle(rawDocument) {
    
    R.values(this.plugins).forEach(plugin => {
      plugin.eachArticle(rawDocument);
    });
  }

  runPluinAfter(rawDocument) {
  ...
  loadDir(dirPath, outputPath, category, cb) {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(this.rootIgnoreRegs);
      ...
      let mappingDirs = self.options.MAPPING || {};

      //subDirs.filter()

      R.splitWhen(dir => mappingDirs.indexOf(dir) > 0, subDirs)
      
      subDirs.filter((subDir) => {

        if( mappingDirs[subDir] ){
    ...
    async.each(category.articles, (articleInfo, asyncCb) => {
      ...
      new Promise((resovle) => {
        ...
        parseToData(filePath, articleRawData, (err, articleDoc) => {
          ...
          self.runPluinEachArticle(articleRawData, {
            title: articleDoc.title,
            outputUrl: outputUrl
          });
