...
export class RenderController {
    ...
    renderCategoryIndex(name, outputPath, articles, cb) {
        ...
        // R.splitEvery(this.options.BLOG.CATEGORY_ARTICLE_NUMBER, sorted).forEach((articleChunk) => {
        //     let data = {
        ...
        //     let outputDir = i === 0 ? outputPath : path.join(outputPath, i + self.themeConfig.PAGING_START_N + '');
            
        //     fsExtra.mkdir(outputPath, (err) => {
    ...
    renderAllArticles(cb) {
        if( this.templates['allarticles'] ){
            ...
            _.chunk(allarticles, this.options.BLOG.ALL_PAGE_ARTICLE_NUMBER).map((articles, i) => {
                ...
                var html = self.renderTemp('allarticles', {
                    title: self.options.BLOG.NAME,
                    articles: articles,
                    categorys: categorys,
                    currentPageN: i + self.themeConfig.PAGING_START_N,
                    pageN: pageN
                });
